...
# from https://gist.github.com/ugate/392185a959737365834f54d5bf4aae5b
...
echo "==========================================================================="
yn=("*** Skip ***" "Regenerate SSH server keys (for SSH, SFTP, etc.)")
yna=
PS3="Enter 1 or 2: "
select yna in "${yn[@]}"; do
  for item in "${yn[@]}"; do
...
dtcInstall() {
  if [[ $dtc != 1 ]]; then
    ...
    # sudo dtc -I dtb -O dts -o /boot/sun5i-r8-chip.dts /boot/sun5i-r8-chip.dtb # decompile DTB
...
# install node.js
echo "==========================================================================="
yn=("*** Skip ***" "Install/Update Node.js")
yna=
PS3="Enter 1 or 2: "
select yna in "${yn[@]}"; do
  for item in "${yn[@]}"; do
...
if [[ $yna == "${yn[1]}" ]]; then
  ...
  if [[ -n "$nodevernew" && "$nodever" == "$nodevernew" ]]; then
